...
class ArrayIterator{
    ...
    public:
        ...
        ArrayIterator& operator++() {_ptr++; return *this;}
...
class MyArray {
    private:
        ...
        int _capacity;
    public:
        MyArray(): _size(0), _capacity(1), _arr(new T[_capacity]){}
        ...
        void push(const T item){
            _arr[_size] = item;
            _size++;
            if(_size ==  _capacity) this->resize(_capacity*2);
        }

        void insert(const int index, const T item){
            if(index > _size) throw std::invalid_argument("index out of size");
        ...
        void pop(){
            _size--;
            if(_size < _capacity/4) this->resize(_capacity/2);
        }
